...
class Club:
    ...
    def get_persons_by_age(self, min_age: int, max_age: int) -> list[Person]:
        ...
        """
        persons: list[Person] = []
        if len(self.__sorted_set) > 0:
            min_id: int = self.__sorted_set[0].id
            left: int = self.__sorted_key_list.bisect_left(Person(min_id, min_age))
            right: int = self.__sorted_key_list.bisect_left(Person(min_id, max_age + 1))
            persons = list(self.__sorted_key_list[left:right])

        return persons

if __name__ == '__main__':
    person1: Person = Person(100, 55)
    person2: Person = Person(500, 25)
    person3: Person = Person(300, 29)
    ...
    print(f"Persons between 20 and 29: {club.get_persons_by_age(20, 29)}")
